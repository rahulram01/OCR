...
import logging
import time
# Import your OCR processing script
# from invoice_processing import generate_gemini_response
...
@app.route('/process-invoice-ocr', methods=['POST'])
def process_invoice_ocr():
    try:
        input_prompt = request.form.get("input_prompt", "")
        image_loc = request.form.get("image_loc", "")
        question_prompts = request.form.getlist("question_prompts")
        ...
        ocr_text = generate_gemini_response(input_prompt, image_loc, question_prompts, output_file_path)

        # Calculate processing time
        end_time = time.time()
        processing_time = end_time - start_time

        # Construct the response with accurate processing times
        return jsonify({
            "success": True,
            "time": {
                "recognition": processing_time,
                "parsing": processing_time
            },
            "ocr_text": ocr_text  # Include the OCR text in the response
        }), 200

    except Exception as e:
        logger.error(f"Error in /process-invoice-ocr route: {str(e)}")
        return jsonify({"error": f"Internal Server Error: {str(e)}"}), 500
